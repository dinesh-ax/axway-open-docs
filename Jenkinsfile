...
// e.g. env.PREVIEW_FOR_BRANCHES = "developaug22 developdec22"test
env.PREVIEW_FOR_BRANCHES = "developmay22 developaug22 developnov22 wingtest"
  ...
  timestamps{  // enable timestamp in the console logs
    ansiColor('xterm') { // using ansi colours
      ...
      try {
        ...
        //  - the GITHUB_OPENDOCS_CREDS is needed to update the PR with links to the build and preview
        stage ('Start Preview') {
          withCredentials([
          ...
          // the _preview_url.txt file is generated by the opendocs-preview.sh script above
          if (fileExists('_preview_url.txt')) {
            ...
            // only send email to the address saved to _preview_email_author.txt ... if this doesn't exist then skip emails
